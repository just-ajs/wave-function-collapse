...
namespace WaveFunctionCollapse
{
    public class PatternFromSampleExtractor
    {
        ...
        {
            var patterns = PatternsFromSampleOverlapping();
            BuildPropagator(patterns);

            return patterns;
        }

        // From provided points create patterns that are result of pairing 4 points that are next to each other
        public List<Pattern> PatternsFromSampleOverlapping()
        {
            // Get states based on unit tile type
            State[,] tileStates = GetTileStates(ConvertToInt(unitElementsOfType0), ConvertToInt(unitElementsOfType1), ConvertToInt(unitElementsCenters));

            // Size of side of input sample (if 5x5, then there is 25 area centers)
            int tileSize = (int)Math.Sqrt(unitElementsCenters.Count());
            ...
            for (int i = 0; i < numberOfSubTiles; i++)
            {
                ...
                var pattern = new Pattern(miniTile, tilesWeights, (int)N);
            ...
            if (rotation)
            {
                ...
                rotatedPatterns = GenerateRotatedTiles(patterns, tilesWeights);
        ...
        public void BuildPropagator(List<Pattern> patternsFromSample)
        {
